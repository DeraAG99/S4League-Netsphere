namespace NeoNetsphere.Network.Service
{
    ...
    internal class AuthService : ProudMessageHandler
    {
        ...
        public async Task EULoginHandler(ProudSession session, LoginEUReqMessage message)
        {
            ...
            using (var db = AuthDatabase.Open())
            {
                ...
                else if (message.token != "")
                {
                    ...
                    if (account != null)
                    {
                        var lastlogin = DateTimeOffset.ParseExact(account.LastLogin, "yyyyMMddHHmmss", CultureInfo.InvariantCulture, DateTimeStyles.None);
            ...
            var sessionId = Hash.GetUInt32<CRC32>($"<{account.Username}+{account.Password}+{session.RemoteEndPoint.Address.MapToIPv4()}+{Config.Instance.API.ApiKey} a>");
            var authsessionId = Hash.GetString<CRC32>($"<{account.Username}+{sessionId}+{datetime} a>");
            var newsessionId = Hash.GetString<CRC32>($"<{authsessionId}+{sessionId} a>");
        ...
        {
            await session.SendAsync(new ServerListAckMessage(server.ServerManager.GetServers()));
        }
