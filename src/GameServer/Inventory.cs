...
namespace NeoNetsphere
{
  internal class Inventory : IReadOnlyCollection<PlayerItem>
  {
    ...
    public PlayerItem Create(CapsuleRewardDto rewardDto)
    {
      ...
      if (price == null)
          throw new ArgumentException($"Price not found for item {rewardDto.ItemNumber} with period type {rewardDto.PeriodType}");
      ...
      var rewardItem = new PlayerItem(this, shopItemInfo, price, rewardDto.Color, itemEffects.ToArray(), DateTimeOffset.UtcNow, 1);
    ...
    internal void Save(IDbConnection db)
    {
        ...
        foreach (var item in _items.Values)
        {
            ...
            catch
            {
             ...
             if (shopItem == null)
             {
                 Log.Error($"[Inventory.Save] ShopItemInfo null for ItemId={item.Id}, ItemNumber={item.ItemNumber.Id}");
                 continue;
             }
            ...
            if (price == null)
            {
                ...
                if (price != null)
                {
                    Log.Warning($"[Inventory.Save] Used fallback price for ItemId={item.Id} Period={item.Period}, fallback to Period={price.Period}");
                }
            }
